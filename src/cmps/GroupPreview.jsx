...
export function GroupPreview({
    ...
    return (
        <>
            {group.isCollapse ? (
                <article
                    className={`group-preview collapse`}
                    style={
                        {
                            /*padding:'0', width:'max-content', height:'100%'*/
                        }
                    }
                >
                    <header style={{ display: 'flex', flexDirection: 'column' }} className="group-header">
                        <button
                            style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}
                            className={`group-menu-button collapse-btn ${group?.style?.backgroundColor?.substr(1)}`}
                            onClick={onCollapse}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" role="presentation">
                                <path
                                    fill={`${group.style.backgroundColor ? colorLuminance(group.style.backgroundColor, -0.55) : 'currentcolor'}`}
                        ...
                        <div onClick={onCollapse} className={`group-header-btn ${group?.style?.backgroundColor?.substr(1)}`}>
                            {title}
                        </div>
                    </header>
                </article>
                ...
                <article className="group-preview">
                    <header className="group-header">
                        {isGroupHeaderEdit ? (
                            ...
                            <div onClick={onEditGroupHeader} className={`group-header-btn ${group?.style?.backgroundColor?.substr(1)}`}>
                                {title}
                            </div>
                        )}
                        {isMenuOpen ? (
                            <GroupMenu
                                ...
                                board={board}
                                group={group}
                                isCopyGroupClicked={isCopyGroupClicked}
                                setIsCopyGroupClicked={setIsCopyGroupClicked}
                                setIsAddTaskClicked={setIsAddTaskClicked}
                        ...
                        <button
                            style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}
                            className={`group-menu-button ${group?.style?.backgroundColor?.substr(1)}`}
                            onClick={toggleMenu}
                        >
